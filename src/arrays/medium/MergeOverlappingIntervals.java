...
public class MergeOverlappingIntervals {
    ...
    public static int[] getMergedInterval(int[] leftInterval , int[] rightInterval){
        ...
        return new int[] {leftIntervalStart , (leftIntervalEnd>rightIntervalEnd)?(leftIntervalEnd):(rightIntervalEnd) };
    }
    ...
    public static int[][] mergeOverlappingIntervals(int[][] intervals) {
        ...
        for(int[] interval : intervals){
            ...
            for(int index=0; index<mergedIntervals.size(); index++){
                ...
                else if(isIntervalOverlap(interval,mergedIntervals.get(index))) {
                    int[] mergedInterval = mergedIntervals.remove(index);
                    mergedIntervals.add(index , getMergedInterval(interval , mergedInterval));
                    overlap=true;
                }
            }
            if(!overlap)
